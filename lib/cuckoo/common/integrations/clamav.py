...
def get_clamav(file_path):
    ...
    if HAVE_CLAMAV and os.path.getsize(file_path) > 0:
        try:
            # Resolve symlinks before scanning. CAPE stores samples under
            # /opt/CAPEv2/storage/ as content-addressed files referenced by
            # symlinks; clamd dereferences the real path but its allmatchscan
            # result dict is keyed by whatever path it actually scanned, so
            # results[file_path] KeyErrors when file_path was a symlink.
            resolved_path = os.path.realpath(file_path)
            cd = pyclamd.ClamdUnixSocket()
            results = cd.allmatchscan(resolved_path)
            if results:
                for entry in results.get(resolved_path, results.get(file_path, [])):
